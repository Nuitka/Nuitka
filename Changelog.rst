...
-  Fix, modules that failed to import, should be retried on next import.
...
-  Fix, constant ``False`` values in right hand side of ``and``/``or``
   conditions were generating wrong code if the left side was of known
   ``bool`` shape too.
...
-  Added support for project options.
   ...
   .. code:: python
      ...
      Refer to the user manual for a table of directives and the variables allowed
      to be used.
...
-  Cleanup language for some warnings and outputs.
   ...
   It was still using old "recursion" language rather than talking about
   "following imports", which is the new one.
