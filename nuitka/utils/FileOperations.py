...
def _getReportPath(filename, prefixes):
    if os.path.isabs(os.path.expanduser(filename)):
        ...
        for prefix_name, prefix_path in prefixes:
            if isFilenameBelowPath(path=prefix_path, filename=abs_filename):
                if isWin32Windows():
                    # The check above resolves junctions and short file names,
                    # so the relative path must be made from the resolved
                    # paths too, or else it would escape the prefix.
                    rel_filename = getFilenameRealPath(abs_filename)
                    rel_prefix_path = getFilenameRealPath(os.path.abspath(prefix_path))
                else:
                    rel_filename = abs_filename
                    rel_prefix_path = prefix_path

                return getNormalizedPathJoin(
                    prefix_name, relpath(path=rel_filename, start=rel_prefix_path)
                )
