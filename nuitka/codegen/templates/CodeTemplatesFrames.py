...
#if PYTHON_VERSION >= 340
%(context_identifier)s->m_frame->f_gen = (PyObject *)%(context_identifier)s;
#endif

Py_CLEAR( %(context_identifier)s->m_frame->f_back );

%(context_identifier)s->m_frame->f_back = PyThreadState_GET()->frame;
Py_INCREF( %(context_identifier)s->m_frame->f_back );

PyThreadState_GET()->frame = %(context_identifier)s->m_frame;
Py_INCREF( %(context_identifier)s->m_frame );

#if PYTHON_VERSION >= 340
%(context_identifier)s->m_frame->f_executing += 1;
#endif
...
%(context_identifier)s->m_frame->f_exc_type = thread_state->exc_type;
if ( %(context_identifier)s->m_frame->f_exc_type == Py_None ) %(context_identifier)s->m_frame->f_exc_type = NULL;
Py_XINCREF( %(context_identifier)s->m_frame->f_exc_type );
%(context_identifier)s->m_frame->f_exc_value = thread_state->exc_value;
Py_XINCREF( %(context_identifier)s->m_frame->f_exc_value );
%(context_identifier)s->m_frame->f_exc_traceback = thread_state->exc_traceback;
Py_XINCREF( %(context_identifier)s->m_frame->f_exc_traceback );
#endif
...
#if PYTHON_VERSION >= 300
Py_CLEAR( %(context_identifier)s->m_frame->f_exc_type );
Py_CLEAR( %(context_identifier)s->m_frame->f_exc_value );
Py_CLEAR( %(context_identifier)s->m_frame->f_exc_traceback );
#endif
