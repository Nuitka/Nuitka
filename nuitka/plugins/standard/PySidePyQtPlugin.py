...
not exist, a list of all available will be given.""",
    ...
    def copyQmlFiles(self, full_name, plugin_dirs, target_plugin_dir):
    ...
    @staticmethod
    def findDLLs(full_name, target_plugin_dir, plugin_dirs):
...
""" % {
    ...
    def considerExtraDlls(self, dist_dir, module):
        ...
        if full_name.getTopLevelPackageName() == self.binding_name:
...
class NuitkaPluginPySide2Plugins(NuitkaPluginQtBindingsPluginBase):
    ...
    def getQtPluginDirs(self):
        if self.qt_dir_plugin_dirs is None:
            command = """\
from __future__ import print_function
from __future__ import absolute_import
...
if os.path.exists(guess_path):
    print("GUESS:", guess_path)
# Anaconda has this, but it seems not automatic.
guess_path = os.path.join(os.path.dirname(PySide2.__file__), "..", "..", "..", "Library", "plugins")
if os.path.exists(guess_path):
    print("GUESS:", guess_path)
"""
            ...
            self.qt_dir_plugin_dirs = result

        return self.qt_dir_plugin_dirs
    ...
    def considerExtraDlls(self, dist_dir, module):
        ...
        if full_name == "PySide2":
            plugin_dirs = self.getQtPluginDirs()
    ...
    def removeDllDependencies(self, dll_filename, dll_filenames):
        # TODO: Share this list with PyQt plugin.
        for values in (self.qt_dir_plugin_dirs,):
            for value in values:
                if dll_filename.startswith(value):
                    for sub_dll_filename in dll_filenames:
                        for badword in (
