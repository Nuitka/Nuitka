...
class ExpressionAttributeLookupFixedItems(ExpressionAttributeLookupFixedBase):
    ...
    def computeExpression(self, trace_collection):
        if self.subnode_expression.hasShapeDictionaryExact():
            result = ExpressionAttributeLookupDictItems(
                expression=self.subnode_expression, source_ref=self.source_ref
            )

            return result, "new_expression", "Items attribute on dict shape resolved."
...
class ExpressionAttributeLookupFixedIteritems(ExpressionAttributeLookupFixedBase):
    ...
    def computeExpression(self, trace_collection):
        if str is bytes and self.subnode_expression.hasShapeDictionaryExact():
            ...
            return (
                result,
                "new_expression",
                "Iteritems attribute on dict shape resolved.",
            )
...
class ExpressionAttributeLookupDictItems(ExpressionAttributeLookupFixedItems):
    """Looking up an attribute of an object.
...
class ExpressionAttributeLookupDictIteritems(ExpressionAttributeLookupFixedItems):
    """Looking up an attribute of an object.
...
_attribute_classes = {
    "items": ExpressionAttributeLookupFixedItems,
    "iteritems": ExpressionAttributeLookupFixedIteritems,
}


def makeExpressionAttributeLookup(expression, attribute_name, source_ref):
    attribute_class = _attribute_classes.get(attribute_name)
