#     Copyright 2020, Batakrishna Sahu, mailto:<Batakrishna.Sahu@suiit.ac.in> find license text at end of file
...
class ZipIterationHandle(IterationHandleBase):
    """Iteration handle for built-in zip values."""
    ...
    def __init__(self, iteration_handles, source_ref):
        self.iteration_handles = iteration_handles
        self.source_ref = source_ref

    def __repr__(self):
        return "<%s of %r>" % (self.__class__.__name__, self.iteration_handles)

    def getNextValueExpression(self):
        if not self.iteration_handles:
            return None

        iteration_values = []

        for iteration_handle in self.iteration_handles:
        ...
        return makeExpressionMakeTupleOrConstant(
            elements=tuple(iteration_values),
            user_provided=False,
            source_ref=self.source_ref,
        )
    ...
    def getIterationValueWithIndex(self, value_index):
        ...
        for iteration_handle in self.iteration_handles:
            iteration_value = iteration_handle.getIterationValueWithIndex(value_index)
    ...
    def getIterationLength(self):
        ...
        for iteration_handle in self.iteration_handles:
            iteration_length = iteration_handle.getIterationLength()
